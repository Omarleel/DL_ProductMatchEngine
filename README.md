...
## Multitenant por sucursal

La API soporta aislamiento por `tenant` para evitar cruces entre sucursales.

- `POST /api/v1/datasets/generar` recibe `tenant`.
- Los datasets de tenants no-default se guardan en:

```text
data/tenants/<TENANT>/raw/maestro.csv
data/tenants/<TENANT>/raw/productos_facturas.csv
```

- Los modelos, candidatos, rechazados y reportes de entrenamiento se guardan en:

```text
artifacts/tenants/<TENANT>/clasificador_productos_v1/
artifacts/tenants/<TENANT>/homologador_productos_v1/
```

- Entrenamiento e inferencia deben enviar el mismo `tenant` para usar el maestro, facturas y modelo de esa sucursal.
