...
def entrenar_y_promover_homologador(
    *,
    ...
    logger.info("Esperando lock de entrenamiento para %s | run_id=%s", model_name, run_id)
    with TrainingLock(artifacts_dir / ".locks" / f"{model_name}.lock"):
        logger.info("Lock obtenido. Construyendo pares base del homologador")
        ...
        logger.info("Split listo: train_base_rows=%s, valid_base_rows=%s", len(train_base), len(valid_base))

        warmup_model = ModeloHomologadorProductos()
        logger.info("Entrenando warmup del homologador")
        warmup_report = warmup_model.fit_on_split(
            train_df=train_base,
            valid_df=valid_base,
            epochs=epochs_warmup,
            batch_size=batch_size,
            phase="homologador warmup",
        )
        logger.info("Warmup finalizado")

        logger.info("Iniciando minería de negativos duros")
        ...
        logger.info("Minería de negativos duros finalizada: rows=%s", len(pares_hard))
        ...
        if not pares_hard.empty:
            ...
            logger.info("Negativos duros guardados: path=%s", pares_hard_path)
            ...
            logger.info("No se encontraron negativos duros; se entrenará solo con pares base")
        ...
        logger.info("Dataset final listo: rows=%s path=%s", len(train_final), train_final_path)

        final_model = ModeloHomologadorProductos()
        logger.info("Entrenando modelo candidato final")
        final_report = final_model.fit_on_split(
            train_df=train_final,
            valid_df=valid_base,
            epochs=epochs_final,
            batch_size=batch_size,
            phase="homologador final",
        )
        logger.info("Entrenamiento final del candidato completado")

        incumbent_report_current = None
        if target_dir.exists():
            logger.info("Evaluando modelo campeón actual en %s", target_dir)
        ...
        logger.info(
            "Entrenamiento finalizado. promoted=%s reason=%s",
            promotion["promoted"],
            promotion["reason"],
        )
