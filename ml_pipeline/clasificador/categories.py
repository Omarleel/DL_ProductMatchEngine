...
class CategoryLexicon:
    ...
    def hits(self, text: str) -> list[str]:
        ...
        for gram in grams:
            categories = self.term_to_categories.get(gram)
            if not categories:
                continue
            gram_len = len(gram.split())
            for cat in categories:
                scores[cat][0] += 1
                scores[cat][1] = max(scores[cat][1], gram_len)

        scored = [(cat, vals[0], vals[1]) for cat, vals in scores.items()]
