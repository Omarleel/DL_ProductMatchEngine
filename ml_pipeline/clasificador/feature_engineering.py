...
def add_item_aux_features(df: pd.DataFrame, category_lexicon: CategoryLexicon | None) -> pd.DataFrame:
    ...
    if "brand_hint" in work.columns:
        work["brand_hint"] = _safe_str_series(work["brand_hint"])
    else:
        work["brand_hint"] = _safe_str_series(raw_text.map(extract_primary_brand))

    if category_lexicon is None:
        work["category_hint"] = _safe_str_series(work.get("category_hint", ""))
    elif "category_hint" in work.columns:
        work["category_hint"] = _safe_str_series(work["category_hint"])
    else:
        category_cache: dict[str, str] = {}

        def _category_hint_cached(text: str) -> str:
            if text not in category_cache:
                category_cache[text] = category_lexicon.primary(text)
            return category_cache[text]

        work["category_hint"] = _safe_str_series(base_text.map(_category_hint_cached))
