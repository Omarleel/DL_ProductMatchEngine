...
def inferir_atributos_producto(
    ...
    df_para_resolvers = productos_facturas.copy()
    if "RucProveedor" in df_para_resolvers:
        df_para_resolvers["RucProveedor"] = df_para_resolvers["RucProveedor"].astype(str).replace(RUC_EQUIVALENTES)
    
    if "CodProducto" in df_para_resolvers:
    ...
    if resolvers and len(df_para_resolvers) > 0:
        ...
        weight_df = resolvers["weight"].resolve_many(
            productos_facturas=df_para_resolvers,
        ...
        if resolver_marca_categoria_desde_maestro:
            bc_df = resolvers["brand_cat"].resolve_many(df_para_resolvers)
    ...
    if include_factor_debug:
        to_concat = []
        for df_source in [factor_df if 'factor_df' in locals() else None, 
                          weight_df if 'weight_df' in locals() else None, 
                          bc_df if 'bc_df' in locals() else None]:
