...
class ModeloClasificadorProductos:
    ...
    def fit_on_split(
        ...
        logger.info(
            f"Preparando datos: train_rows={len(train_df)}, valid_rows={len(valid_df)}, "
        ...
        callbacks = [
            logger.keras_epoch_callback(
                ...
                metric_keys=[
                    ...
                    "val_loss",
                    "marca_output_acc",
                    "val_marca_output_acc",
                    "categoria_output_acc",
                    "val_categoria_output_acc",
                ],
            ),
            tf.keras.callbacks.EarlyStopping(monitor="val_loss", patience=5, restore_best_weights=True, verbose=0),
            tf.keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=2, min_lr=1e-5, verbose=0),
        ]

        logger.info("Iniciando model.fit")
        ...
        logger.info("Evaluando set de validación")
        metrics_valid = self.evaluate(valid_df, batch_size=batch_size)
        resumen_metricas = ", ".join(f"{k}={v:.4f}" for k, v in metrics_valid.items() if isinstance(v, (int, float)))
        logger.info(f"Evaluación finalizada{(': ' + resumen_metricas) if resumen_metricas else ''}")
